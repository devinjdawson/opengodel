...
[←PreviousPDFSettings](https://godelterminal.com/docs/commands/pdf.html)[NextNOTENotes→](https://godelterminal.com/docs/commands/note.html)
...
AL is the Godel Terminal command for creating desktop alerts on any security's price, volume, or intraday change.

![AL command showing active desktop price alerts with ticker, threshold, and trigger conditions](./AL Command (Alerts) _ Godel Terminal Docs_files/al-01.png)
...
![Launches the Alerts command](./AL Command (Alerts) _ Godel Terminal Docs_files/al-02.png)

Launches the Alerts command

## Setting Up Alerts

1. **Search for a ticker** in the search field (e.g., "NVDA") and s**elect the security** from the dropdown showing available exchanges and asset classes

![AL command showing the alert creation dialog for setting a new price alert on a security](./AL Command (Alerts) _ Godel Terminal Docs_files/al-03.png)

2. **Configure alert parameters:**

![AL command showing the alert creation dialog for setting a new price alert on a security](./AL Command (Alerts) _ Godel Terminal Docs_files/al-04.png)

* **Price:** Set your trigger value
* **Condition:** Choose trigger type using the dropdown (&gt; greater than, &lt; less than, = equals)
* **Change/Change %:** Set alerts based on price change or percentage change

3. **Sound:** Select notification sound from dropdown:

![AL command showing the alert creation dialog for setting a new price alert on a security](./AL Command (Alerts) _ Godel Terminal Docs_files/al-05.png)

4. Click **Save Alerts** to activate

## Browser Permissions

When first using AL, your browser will request notification permissions:

* Click **Allow** to enable desktop notifications
* Notifications can be managed in your browser's Settings

## Alert Behavior

* **One-time trigger:** Alerts fire once when the condition is met, then automatically clear
* **Desktop notification:** Shows ticker, price, and threshold when triggered

## Related commands

* [QM Quote Monitor](https://godelterminal.com/docs/commands/qm.html)
* [FOCUS Focus](https://godelterminal.com/docs/commands/focus.html)

## FAQ

What does AL do?

AL is the Godel Terminal alerts command.

How do I open AL in Godel Terminal?

Type AL in the terminal, or prefix with a ticker (for example, NVDA US EQ AL).

Is AL available on all plans?

Yes, AL is available on every Godel plan.

Does AL work for ETFs, indices, or non-US securities?

Yes. AL works for ETFs, indices, and non-US securities, not just US stocks.
...
[Get a demo](https://godelterminal.com/docs/commands/al#demo)[Login](https://app.godelterminal.com/?page=login)[Sign Up](https://app.godelterminal.com/?page=register)
