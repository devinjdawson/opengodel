...
[←PreviousNOTENotes](https://godelterminal.com/docs/commands/note.html)[NextCHANGEChangelog→](https://godelterminal.com/docs/commands/change.html)

[Docs](https://godelterminal.com/docs.html)/
[Commands](https://godelterminal.com/docs.html#commands)/
ENT

# ENT: Entitlements Command

ENT is the Godel Terminal command for managing add-on exchange feeds and premium features layered on top of your base Godel subscription.
...
![Godel Terminal command bar showing ENT typed with the COMMANDS autocomplete suggesting 'Entitlements for exchange feeds and features on Godel'](./ENT Command (Entitlements) _ Godel Terminal Docs_files/ent-01.png)

## Reading the List

Entitlements are shown in a single table:

| Column | Description |
| --- | --- |
| **Entitlement** | Name of the feed or feature. A pulsing green dot means it's active on your account; a gray dot means it's not. |
| **Status** | **Active** (green pill) or **Inactive** (gray pill). |

The list is sorted so your active entitlements come first, then entitlements you can self-subscribe to, then everything else. Use the search box at the top of the window to fuzzy-match by name.

## Entitlement Details

Click any row to expand it. The expanded panel shows:

* The entitlement's **description**.
* Every available **price** with its currency, amount per billing interval (e.g. "$X per month" or "$X per 12 months"), and whether it is the **Retail** or **Professional** rate.
* A toggle to **subscribe** or **unsubscribe** (only visible if the entitlement supports self-service).

## Subscribing & Unsubscribing

* Flip an **inactive** toggle to on: a confirmation toast notes that you will be billed the prorated amount immediately. Confirm to add.
* Flip an **active** toggle to off: a confirmation toast asks you to confirm removal.
* After adding or removing an entitlement, the window reloads after a short delay so the change is reflected everywhere.

## Pending State

If a subscription is still being processed, the toggle is replaced with a yellow **PENDING** label until the change completes.

## Notes

* ENT is in beta.
* ENT is limited to a single open window.
* Users on a custom trial cannot use ENT: the window shows a notice instead of the list.
* Some entitlements are managed by Godel support and cannot be self-subscribed; these still appear in the list so you can see their status.

## Related commands

* [ACM Account Management](https://godelterminal.com/docs/commands/acm.html)
* [BROK Brokerage](https://godelterminal.com/docs/commands/brok.html)
* [PDF Settings](https://godelterminal.com/docs/commands/pdf.html)

## FAQ

What does ENT do?

ENT is the Godel Terminal entitlements command.

How do I open ENT in Godel Terminal?

Type ENT in the terminal.

Is ENT available on all plans?

ENT requires a paid subscription. It is not available on free (piker) or trial accounts.
...
[Get a demo](https://godelterminal.com/docs/commands/ent#demo)[Login](https://app.godelterminal.com/?page=login)[Sign Up](https://app.godelterminal.com/?page=register)
