...
[←PreviousWEIWorld Equity Index](https://godelterminal.com/docs/commands/wei.html)[NextIMAPIntraday Market Map→](https://godelterminal.com/docs/commands/imap.html)

[Docs](https://godelterminal.com/docs.html)/
[Commands](https://godelterminal.com/docs.html#commands)/
WEIF

# WEIF: World Equity Index Futures Command

WEIF is the Godel Terminal command for monitoring futures prices on the world's major equity indices.
...
![Godel Terminal command bar showing WEIF typed with the COMMANDS autocomplete suggesting 'Monitor and compare futures prices for the world's equity indices'](./WEIF Command (World Equity Index Futures) _ Godel Terminal Docs_files/weif-01.png)

The WEIF window lists index futures grouped by region, with live price, change, year-to-date performance, and delay status for each contract:

![WEIF World Equity Index Futures window showing Americas index futures (S&P 500 E-Mini, Dow Futures Mini, Nasdaq 100 E-Mini, Russell 2000 E-Mini) with Last, Chg, Chg %, YTD %, and Delay columns](./WEIF Command (World Equity Index Futures) _ Godel Terminal Docs_files/weif-02.png)

## Display Columns

| Column | Description |
| --- | --- |
| **Name** | Index futures contract name |
| **Value** | Last traded price |
| **Chg** | Absolute change vs. previous close |
| **Chg %** | Percentage change vs. previous close |
| **YTD %** | Year-to-date performance |
| **Delay** | D if the quote is delayed; blank if real-time |

## Regions

Contracts are grouped by region. Currently shown: **Americas**. (Additional regions such as EMEA and Asia/Pacific may be enabled over time.)

## Navigation

Click any ticker to open its default view (chart by default). The click-through behavior can be customized in settings via [PDF](https://godelterminal.com/docs/commands/pdf.html "PDF command reference").

## Flash on Update

Rows briefly flash the primary theme color whenever the underlying quote updates, so you can catch movement at a glance.

## Entitlements

WEIF requires a paid Godel account. Free (piker) users are blocked from opening the window.
...
WEIF is the Godel Terminal world equity index futures command.

How do I open WEIF in Godel Terminal?

Type WEIF in the terminal.

Is WEIF available on all plans?

WEIF requires a paid Godel plan. Free (piker) accounts can't open the WEIF window.

Does WEIF work for ETFs, indices, or non-US securities?

WEIF is built specifically for equity index futures, not individual stocks or ETFs. It covers the world's major index futures contracts (currently the Americas, with additional regions enabled over time).
...
[Get a demo](https://godelterminal.com/docs/commands/weif#demo)[Login](https://app.godelterminal.com/?page=login)[Sign Up](https://app.godelterminal.com/?page=register)
